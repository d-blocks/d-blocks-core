    ...
    *,
        ...
        if repo is None:
            message = "\n".join(
                [
                    ...
                    "Either run 'dblc init' (recommended) or 'git init'.",
                ]
            )
    ...
    for i, obj in enumerate(in_scope, start=1):
        ...
        if repo is not None and prev_db is not None and db != prev_db:
            if commit and not repo.is_clean():
                repo.add()
                repo.commit(f"dblc env-extract {env_name}: {db}")
    ...
    if repo is not None:
        if not repo.is_clean():
            if commit:
                repo.add()
                repo.commit(f"dblc env-extract {env_name}: delete dropped objects")
            else:
...
#             if not repo.is_clean():
#                 repo.add()
#                 repo.commit(f"dblc env-extract {env_name}: {database.database_name}")
